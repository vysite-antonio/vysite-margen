...
export function extractCurrentValue(
  ...
  if (objective.scope_type === 'all') {
    ...
    if (objective.metric === 'revenue')
      return Number(kpisExtendedData.total_ventas ?? 0)
    if (objective.metric === 'margin')
      return Number(kpisExtendedData.total_margen ?? 0)
    if (objective.metric === 'new_clients')
      return Number(kpisExtendedData.clientes_activos ?? 0)
    return 0
  }
  ...
  if (objective.scope_type === 'category') {
    // Suma de las categorías seleccionadas
